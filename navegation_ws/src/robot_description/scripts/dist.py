...
class RobotHeightScan(Node):
    def __init__(self):
        ...
        self.pitch_min_deg = float(self.declare_parameter('pitch_min_deg',  5.0).value)
        self.pitch_max_deg = float(self.declare_parameter('pitch_max_deg', 15.0).value)
        self.yaw_min_deg   = float(self.declare_parameter('yaw_min_deg', -90.0).value)
        self.yaw_max_deg   = float(self.declare_parameter('yaw_max_deg',  90.0).value)
        self.bin_width_deg = float(self.declare_parameter('bin_width_deg', 1.0).value)
        ...
        # self.ct y self.st contienen cos(-tilt_rad) y sin(-tilt_rad)
        ...
        # ---------- QoS: Asegurar compatibilidad con Cartographer ----------
        ...
        output_qos = QoSProfile(
            depth=10,
            reliability=ReliabilityPolicy.RELIABLE, # CRÍTICO para Cartographer
            durability=DurabilityPolicy.VOLATILE
        )

        self.sub = self.create_subscription(PointCloud2, self.input_cloud, self.cb, sensor_qos)
        self.pub_scan = self.create_publisher(LaserScan, self.scan_topic, output_qos)
        self.pub_front = self.create_publisher(Float32, '/obstacle_distance_front', output_qos)

        self.get_logger().info(
            f"PointCloud2 ({self.input_cloud}) -> LaserScan ({self.scan_topic}) | "
            f"QoS=Reliable | frame={self.scan_frame}"
        )

    def cb(self, msg: PointCloud2):
        span = max(1e-6, self.yaw_max - self.yaw_min)
        num_bins_cb = int(math.floor(span / self.bin_w)) + 1
        bins = [[] for _ in range(num_bins_cb)]
        ...
        try:
            ...
            for i, (x, y, z) in enumerate(points):
                ...
                # =========================================================
                # 1. Rotación 3D para la orientación de la cámara
                # Rotación de -90 grados alrededor del eje Y
                # =========================================================
                # xp (Nuevo X = Vertical/Arriba-Abajo)
                xp = x * self.ct + z * self.st  
                # yp (Nuevo Y = Lateral/Izquierda-Derecha)
                yp = y                          
                # zp (Nuevo Z = Profundidad/Adelante-Atrás)
                zp = -x * self.st + z * self.ct 
                # =========================================================

                # 🔑 2. CÁLCULO DEL YAW (Horizontalidad): atan2(Y_comp, X_comp)
                # El nuevo plano horizontal es (zp, yp), donde zp es la profundidad (Eje X)
                # y yp es el lateral (Eje Y).
                yaw   = math.atan2(yp, zp)
                
                # 🔑 3. CÁLCULO DE LA DISTANCIA HORIZONTAL: Distancia en el plano (zp, yp)
                horiz = math.hypot(zp, yp) 
                
                # 🔑 4. CÁLCULO DEL PITCH (Verticalidad): atan2(Z_comp, Horizontal_dist)
                # El componente vertical es xp
                pitch = math.atan2(xp, horiz) 
        ...
        ranges = [float('inf')] * num_bins_cb
        for i in range(num_bins_cb):
            if bins[i]:
...
#         # ---------- Parámetros ----------
#         # Nota: Los valores iniciales aquí son de referencia y serán sobreescritos por el launch file.
#         self.scan_frame = self.declare_parameter('scan_frame', 'base_scan') \
#                             .get_parameter_value().string_value
#         self.input_cloud   = self.declare_parameter('input_cloud', '/points2').get_parameter_value().string_value
#         self.scan_topic    = self.declare_parameter('scan_topic',  '/scan').get_parameter_value().string_value
...
#                 # =========================================================
#                 # MODIFICACIÓN CLAVE: Compensación de Rotación en Y 
#                 # (Rotación de -90 grados alrededor del eje Y)
#                 # =========================================================
                
#                 # xp (Nuevo X, adelante) = x * cos(-t) + z * sin(-t)
#                 xp = x * self.ct + z * self.st  
#                 # yp (Nuevo Y, izquierda) = y
#                 yp = y                          
#                 # zp (Nuevo Z, arriba) = -x * sin(-t) + z * cos(-t)
#                 zp = -x * self.st + z * self.ct 
#                 # =========================================================
